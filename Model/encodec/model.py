...
import Model.encodec.quantization as qt
import Model.encodec.modules as m
from Model.encodec.utils import _check_checksum, _linear_overlap_add, _get_checkpoint_url
...
class EncodecModel(nn.Module):
    ...
    def encode(self, x: torch.Tensor) -> tp.List[EncodedFrame]:
        ...
        for offset in range(0, length, stride):
            ...
            tmp = self._encode_frame(frame)
            encoded_frames.append(tmp)
    ...
    def get_code(self, x: torch.Tensor):
        """Get the code representation of the input `x`.
        """
        frames = self.encode(x)
        codes = []
        for emb, _ in frames:
            qv = self.quantizer.forward(emb, self.sample_rate, self.bandwidth)
            codes.append(qv.quantized)
        return codes
